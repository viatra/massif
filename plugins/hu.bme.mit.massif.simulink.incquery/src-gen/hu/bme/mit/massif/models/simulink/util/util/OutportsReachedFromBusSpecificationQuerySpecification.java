...
public final class OutportsReachedFromBusSpecificationQuerySpecification extends BaseGeneratedEMFQuerySpecification<OutportsReachedFromBusSpecificationMatcher> {
  private OutportsReachedFromBusSpecificationQuerySpecification() {
  ...
  private static class GeneratedPQuery extends BaseGeneratedEMFPQuery {
    private final static OutportsReachedFromBusSpecificationQuerySpecification.GeneratedPQuery INSTANCE = new GeneratedPQuery();
    ...
    public Set<PBody> doGetContainedBodies() throws QueryInitializationException {
      ...
      try {
      {
      	...
      	new BinaryTransitiveClosure(body, new FlatTuple(var_outPort, var_endPortBeforeNextSpec), SimpleConnectedOutPortQuerySpecification.instance().getInternalQueryRepresentation());
      	new PositivePatternCall(body, new FlatTuple(var_endPortBeforeNextSpec, var_inPort, var__pc), PortConnectionQuerySpecification.instance().getInternalQueryRepresentation());
      ...
      }
      {
      	...
      	new TypeUnary(body, var_outPort, getClassifierLiteral("http://hu.bme.mit.massif/simulink/1.0", "OutPort"), "http://hu.bme.mit.massif/simulink/1.0/OutPort");
      	new TypeUnary(body, var_nextSpec, getClassifierLiteral("http://hu.bme.mit.massif/simulink/1.0", "BusSpecification"), "http://hu.bme.mit.massif/simulink/1.0/BusSpecification");
      	new PositivePatternCall(body, new FlatTuple(var_outPort, var_inPort, var__pc), PortConnectionQuerySpecification.instance().getInternalQueryRepresentation());
      	new TypeBinary(body, CONTEXT, var_nextSpec, var_inPort, getFeatureLiteral("http://hu.bme.mit.massif/simulink/1.0", "Block", "ports"), "http://hu.bme.mit.massif/simulink/1.0/Block.ports");
      	bodies.add(body);
      }
