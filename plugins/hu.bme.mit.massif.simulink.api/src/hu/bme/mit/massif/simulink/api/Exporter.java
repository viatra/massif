...
public class Exporter {
	...
	private Double createConnectionFromSingleConnection(SingleConnection singleConnection, OutPort outPort) {
		...
		for (Port port : outPortContainerPortList) {
			if (port instanceof OutPort) {
				...
				if (port.equals(outPort)) {
					...
					// only count up at normal Simulink outports
					if(port.getPortType() == SimscapePortType.NONE) {
		...
		int fromPortNumber = 1;

		// add the identifier for Simscape connections
		String srcPortIdentifier;
		if(outPort.getPortType() == SimscapePortType.LCONN) {
			srcPortIdentifier = "/LConn";
			fromPortNumber = outPort.getPortNo();
		}
		else if(outPort.getPortType() == SimscapePortType.RCONN) {
			srcPortIdentifier = "/RConn";
			fromPortNumber = outPort.getPortNo();
		}
		else {
			// 'normal' simulink connection
			srcPortIdentifier = "/";
			fromPortNumber = indexOfOutportInMatlab;
		}

		String srcPort = fromBlockName + srcPortIdentifier + fromPortNumber;
		...
		// add the identifier for Simscape connections
		String dstPortIdentifier;
		if(targetInPort.getPortType() == SimscapePortType.LCONN) {
			dstPortIdentifier = "/LConn";
			toPortNumber = String.valueOf(targetInPort.getPortNo());
		}
		else if(targetInPort.getPortType() == SimscapePortType.RCONN) {
			dstPortIdentifier = "/RConn";
			toPortNumber = String.valueOf(targetInPort.getPortNo());
		}
		else {
			// 'normal' simulink connection
			dstPortIdentifier = "/";
			toPortNumber = String.valueOf(indexOfInportInMatlab);
		}

		String dstPort = toBlockName + dstPortIdentifier + toPortNumber;
