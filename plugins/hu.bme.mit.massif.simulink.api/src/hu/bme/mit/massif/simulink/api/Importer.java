...
public class Importer {
	...
	public void traverseAndCreateEMFModel(ImportMode importMode) throws SimulinkApiException {
		...
		// Create List of List to save all destPort handles of the Simscape outPorts
		List<List<IVisitableMatlabData>> outPortDstPort = new ArrayList<List<IVisitableMatlabData>>();
		// Create List to get the outPortHandles for the dstPorts
		List<IVisitableMatlabData> outPortHandles = new ArrayList<IVisitableMatlabData>();
		...
		// Create ports
		createPorts(outPortDstPort, outPortHandles);

		// Create connections
		createConnections(outPortDstPort, outPortHandles);
	 ...
	 * 
	 * @param outPortDstPort
	 * 			List to save dstPort Handles of Simscape outPorts
	 * @param outPortHandles
	 * 			List to save Simscape outPort Handles
	 */
	private void createPorts(List<List<IVisitableMatlabData>> outPortDstPort, List<IVisitableMatlabData> outPortHandles) {
		...
		// Create List to save all destinations
		List<IVisitableMatlabData> destinationStore = new ArrayList<IVisitableMatlabData>();
		...
		for (Block parentBlock : blocks.values()) {
			...
			// Process Simscape connections
			createSimscapeBlockPorts(portProvider, destinationStore, parentBlock, outPortDstPort, outPortHandles);
			...
			for (IVisitableMatlabData iVisitableMatlabData : outportHandles) {
				...
				createAndAddPort(parentBlock, portProvider, outportHandle, "Outport", 0, "None");
			}
			...
			for (IVisitableMatlabData iVisitableMatlabData : inportHandles) {
				...
				createAndAddPort(parentBlock, portProvider, inportHandle, "Inport", 0, "None");
			}
			...
			for (IVisitableMatlabData iVisitableMatlabData : triggerportHandles) {
				...
				createAndAddPort(parentBlock, portProvider, triggerportHandle, "Trigger", 0, "None");
			}
			...
			for (IVisitableMatlabData iVisitableMatlabData : enableportHandles) {
				...
				createAndAddPort(parentBlock, portProvider, enableportHandle, "Enable", 0, "None");
			}
		}
	...
	/**
	 * Manages inPort/outPort creation for Simscape blocks
	 * 
	 * @param portProvider
	 *            the provider object for the port adapters
	 * @param destinationStore
	 * 			  Storage for the port handles to decide about outPort/inPort creation
	 * @param parentBlock
	 ...
	 * 			  List to save Simscape outPort Handles
	 */
	private void createSimscapeBlockPorts(PortProvider portProvider, List<IVisitableMatlabData> destinationStore,
			Block parentBlock, List<List<IVisitableMatlabData>> outPortDstPort, List<IVisitableMatlabData> outPortHandles) {
		logger.debug("Creating Simscape Ports...");

		// don't create Ports for Simscape Subblocks
		// TODO: implement algorithm for creating and connecting Simscape Subblock Ports
		// see also todo at createAndAddPort
		...
		List<IVisitableMatlabData> lConnHandles = CellMatlabData.getCellMatlabDataData(portHandles.getData("LConn"));
		List<IVisitableMatlabData> rConnHandles = CellMatlabData.getCellMatlabDataData(portHandles.getData("RConn"));

		// check if Ports existing
		if(!lConnHandles.isEmpty() || !rConnHandles.isEmpty()) {

			List<Integer> noConnectionPorts = new ArrayList<Integer>();
			int ic = 1, il = 0, ir = 0;
			
			// Check if Connection existing for every lConn, if not: save Number in noConnectionPorts and create outPort
			for (IVisitableMatlabData iVisitableMatlabData : lConnHandles) {
				Handle lConnHandle = Handle.asHandle(iVisitableMatlabData);
				
				MatlabCommand getLineHandles = commandFactory.getParam().addParam(lConnHandle).addParam("Line");
				IVisitableMatlabData lineHandle = getLineHandles.execute();
				
				if(Handle.getHandleData(lineHandle) < -0.9) {
					MatlabCommand getPortNumber = commandFactory.getParam().addParam(lConnHandle).addParam("PortNumber");
					Integer portNumber = Handle.getHandleData((getPortNumber.execute())).intValue();
					
					createAndAddPort(parentBlock, portProvider, lConnHandle, "Outport", ic, "LConn" + portNumber.toString());
					noConnectionPorts.add(ic);
					il++;
				}
				ic++;
			}
			// Check if Connection existing for every rConn, if not: save Number in noConnectionPorts and create outPort
			for (IVisitableMatlabData iVisitableMatlabData : rConnHandles) {
				Handle rConnHandle = Handle.asHandle(iVisitableMatlabData);
				
				MatlabCommand getLineHandles = commandFactory.getParam().addParam(rConnHandle).addParam("Line");
				IVisitableMatlabData lineHandle = getLineHandles.execute();
				
				if(Handle.getHandleData(lineHandle) < -0.9) {
					MatlabCommand getPortNumber = commandFactory.getParam().addParam(rConnHandle).addParam("PortNumber");
					Integer portNumber = Handle.getHandleData((getPortNumber.execute())).intValue();
					
					createAndAddPort(parentBlock, portProvider, rConnHandle, "Outport", ic, "RConn" + portNumber.toString());
					noConnectionPorts.add(ic);
					ir++;
				}
				ic++;
			...
			// get Connectivity struct array
			MatlabCommand getPortConnectivity = commandFactory.customCommand("portConn = get_param('" + parentBlockFQN + "', 'PortConnectivity')", 0);
			getPortConnectivity.execute();

			// Obtain number of structs
			MatlabCommand getNumberOfStructs = commandFactory.customCommand("numel(portConn)", 1);
			Integer numberOfStructs = Handle.getHandleData(getNumberOfStructs.execute()).intValue();

			int simscapePortNumberOut = 1 + noConnectionPorts.size(), simscapePortNumberIn = 1;

			for (int i = 1; i <= numberOfStructs; i++) {
				// Ports with no Connection are already processed
				if(noConnectionPorts.contains(i)) {
					continue;
				}
				
				// Obtain the content of every struct
				MatlabCommand getConnectivityStruct = commandFactory.customCommand("portConn(" + i + ")", 1);
				StructMatlabData connectivityStruct = StructMatlabData.asStructMatlabData(getConnectivityStruct.execute());

				// Obtain Simscape Port Type and all destinations
				String portType = MatlabString.getMatlabStringData(connectivityStruct.getData("Type"));
				List<IVisitableMatlabData> dstPorts = CellMatlabData.getCellMatlabDataData(connectivityStruct.getData("DstPort"));

				// Process LConns
				if(portType.contains("LConn")) {
					Handle currentHandle = Handle.asHandle(lConnHandles.get(il));

					// check if current Port already exists in EMF-model
					boolean check = false;
					for (int j = 0; j < destinationStore.size(); j++) {
						double store = Handle.asHandle(destinationStore.get(j)).getData();
						double compare = currentHandle.getData();
						if(store == compare) {
							check = true;
							break;
						}
					...
					if (!check) {
						// if not: create outPort and connections
						createAndAddPort(parentBlock, portProvider, currentHandle, "Outport", simscapePortNumberOut, portType);
						simscapePortNumberOut++;

						// save dstPorts and outPortHandle for creating connections later
						outPortDstPort.add(dstPorts);
						outPortHandles.add(currentHandle);

					} else {
						// create inPort
						createAndAddPort(parentBlock, portProvider, currentHandle, "Inport", simscapePortNumberIn, portType);
						simscapePortNumberIn++;
					}
					il++;
				}
				// Process RConns
				if(portType.contains("RConn")) {
					Handle currentHandle = Handle.asHandle(rConnHandles.get(ir));
				...
				// save all destination ports
				destinationStore.addAll(dstPorts);
			}
		}
		logger.debug("Simscape Ports created");
	}
	 ...
	 * @param simscapePortNumber
	 *            the inport/outport Number of Simscape Blocks for the .simulink model
	 * @param simscapeType
	 * 			  the Simscape Port Type with Number (LConn1, ...)
	 */
	private void createAndAddPort(Block parent, PortProvider portProvider, Handle portHandle, String portType,
			int simscapePortNumber, String simscapeType) {
		...
		String isSimscape = "";
		if(simscapePortNumber > 0) {
			portNumber = simscapePortNumber;
			isSimscape = "s";
		} else {
			MatlabCommand getPortNumber = commandFactory.getParam().addParam(portHandle).addParam("PortNumber");
			portNumber = Handle.getHandleData(getPortNumber.execute()).intValue();
		}
		Port port;
		...
		if ("outport".equalsIgnoreCase(portType) || "state".equalsIgnoreCase(portType)) {
			...
			createAndSetSimulinkRef("outport." + isSimscape + portNumber.toString(), parent.getSimulinkRef(), port);
			...
			createAndSetSimulinkRef("inport." + isSimscape + portNumber.toString(), parent.getSimulinkRef(), port);
		}

		// Tells if the block contains created and registered subblocks
		boolean isParent = parents.contains(parent);
		// Avoid connecting Ports inside "Simscape-Subsystems" (Error because BlockSet = null)
		// TODO implement correct importing of Subsystems with Simscape connections (if possible)
		if (isParent && simscapeType == "None") {
		...
		// Set Simscape Port Type and Port Number
		if(simscapeType == "None") {
			port.setPortType(SimscapePortType.NONE);
			// for normal Simulink Blocks portNo = 0
			port.setPortNo(0);
		} else {
			port.setPortType(SimscapePortType.get(simscapeType.substring(0, 5)));
			// for Simscape Blocks portNo is included in simscapeType
			port.setPortNo(Integer.parseInt(simscapeType.substring(5)));
		}
	}
	 ...
	 * @throws SimulinkApiException
	 */
	private void createConnections(List<List<IVisitableMatlabData>> outPortDstPort,
			List<IVisitableMatlabData> outPortHandles) throws SimulinkApiException {
		...
		for (OutPort outp : outPorts.values()) {
			...
			if (Handle.getHandleData(lineHandle) > -0.9) {
				...
				// Process Simscape Connections
				if(outp.getPortType() != SimscapePortType.NONE) {
					// compare outHandle with outPortHandle List
					for (int i = 0; i < outPortHandles.size(); i++) {
						double oPH = Handle.asHandle(outPortHandles.get(i)).getData();

						// if contains: create Line to every destination Port
						if(oPH == outHandle) {
							for(IVisitableMatlabData data2 : outPortDstPort.get(i)) {
								Handle dstPortHandleSimscape = Handle.asHandle(data2);
								//create Line
								createLine(outp, dstPortHandleSimscape, lineHandle);
							}
							break;
						}
					}
				} else {
					// Process normal Simulink connections
					...
					for (IVisitableMatlabData data : CellMatlabData.getCellMatlabDataData(inputPortHandles)) {
						...
						// create Line
						createLine(outp, dstPortHandle, lineHandle);
					}
				}
			}
			// else {
	 ...
	 * @param lineHandle
	 */
	private void createLine(OutPort outp, Handle dstPortHandle, IVisitableMatlabData lineHandle) {
