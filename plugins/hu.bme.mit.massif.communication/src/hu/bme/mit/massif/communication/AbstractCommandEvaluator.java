...
public class AbstractCommandEvaluator<Access extends ICommandAccess> implements ICommandEvaluator {
    ...
    protected final Access commandAccess;

    public AbstractCommandEvaluator(Access access) {
    ...
    public IVisitableMatlabData evaluateCommand(String command, int nargout) {
        ...
        try {
            ...
            if (nargout > 1) {
                ...
                commandAccess.executeEval(resultString + " = " + command, 0);
                commandAccess.executeEval("ImporterTmpResult = " + resultString.replace("[", "{").replace("]", "}"), 0);
                result = dataRetriever();
            } else if (nargout == 1) {
                commandAccess.executeEval("ImporterTmpResult = " + command, 0);
                result = dataRetriever();
            } else {
                commandAccess.executeEval(command, 0);
                result = null;
            }
    ...
    private IVisitableMatlabData processStruct(String structName, Object[] data) throws Exception {

        // //////
        // DEBUG
        // Get all data in the struct with the fieldnames
        Object[] _data = (Object[]) commandAccess.executeEval(structName + "(1:end)", 1)[0];
        ...
        if (structDatas.length <= 0) {
            ...
            for (int i = 0; i < structDatas.length; i++) {
                for (int j = 0; j < ((String[]) _data[0]).length; j++) {
                    ...
                    String type = (String) commandAccess.executeEval("class(" + currVarName + ")", 1)[0];
    ...
    private IVisitableMatlabData processNonStruct(String name, String type, Object data) throws Exception {
        IVisitableMatlabData result = null;
